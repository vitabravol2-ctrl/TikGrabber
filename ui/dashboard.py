...
class DashboardWindow(QMainWindow):
    ...
    def render(self, snap: MarketSnapshot, sim: SimulationState) -> None:
        ...
        self.position_rows["BEST DIRECTION"].setText(f"BEST DIRECTION: {best_dir} | MODE {snap.position_mode}")
        self.position_rows["PHASE"].setText(f"PHASE: {snap.market_phase}")
        self.position_rows["TRAP"].setText(f"TRAP: {snap.trap_score:.0f}")
        ...
        self.position_rows["EXPECTED MOVE"].setText(f"REAL MOVE: {snap.expected_move_ticks_real:.1f} ticks / ${snap.expected_move_usdt_real:.2f} | REQ MOVE: ${snap.required_move_usdt:.2f} | GAP: ${snap.expected_move_usdt_real - snap.required_move_usdt:+.2f}")
